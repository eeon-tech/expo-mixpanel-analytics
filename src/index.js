...
export class ExpoMixpanelAnalytics {
  ...
  register(props) {
    ...
    try {
      AsyncStorage.setItem(ASYNC_STORAGE_KEY, serialize(props))
    } catch (error) {}
  }
  ...
  reset() {
    this.identify(this.clientId)
    try {
      AsyncStorage.setItem(ASYNC_STORAGE_KEY, serialize({}))
    } catch (error) {}
  }
  ...
  _pushEvent(event) {
    ...
    const buffer = new Buffer(serialize(data)).toString('base64')

    return fetch(`${MIXPANEL_API_URL}/track/?data=${buffer}`)
  }

  _pushProfile(data) {
    data = new Buffer(serialize(data)).toString('base64')
